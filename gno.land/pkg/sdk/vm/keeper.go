...
func (vm *VMKeeper) buildCLAError(ctx sdk.Context, creator crypto.Address, sysCLAPkg string) error {
	...
	// Query CLA realm state.
	if rtvs, err := vm.queryEvalInternal(ctx, sysCLAPkg, "requiredHash"); err == nil && len(rtvs) == 1 && rtvs[0].T.Kind() == gno.StringKind {
		claErr.Hash = rtvs[0].GetString()
	}
	if rtvs, err := vm.queryEvalInternal(ctx, sysCLAPkg, "claURL"); err == nil && len(rtvs) == 1 && rtvs[0].T.Kind() == gno.StringKind {
		claErr.URL = rtvs[0].GetString()
	}

	return ErrCLAUnsigned(claErr)
}
